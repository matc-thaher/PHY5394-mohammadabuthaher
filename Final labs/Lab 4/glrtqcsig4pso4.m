function [fitVal,varargout] = glrtqcsig4pso4(xVec, params)
...
[~,qc] = ssrqc(xVec,params);
%Return qc signal
if nargout > 2
    varargout{2}=qc;
end
%log likelihood ratio after maximizing over amplitude parameter
function [ssrVal,qc] = ssrqc(x, params)
%Generate normalized quadratic chirp
phaseVec = x(1).*params.dataX + x(2).*params.dataXSq + x(3).*params.dataXCb;
qc1 = sin(2.*pi.*phaseVec);

% Compute norm for colored data and create color data
[qc,~] = normsig4psd(qc1, params.sampFreq, params.psdVal, 1);
% Colored QC
%qc =  qc1./normFac;
